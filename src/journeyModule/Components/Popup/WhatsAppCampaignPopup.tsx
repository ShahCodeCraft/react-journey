...
export default function WhatsAppCampaignPopup({selectedCampaign, userSelectedNode, handleSubmitWhatsAppCampaignFun }: PopupProps) {
...
// console.log("selectedCampaign"+JSON.stringify(selectedCampaign))
    ...
    const handleNext = async () => {
        ...
        if (activeStep === 2) {
            ...
            if (selectedCampaign) {
                try {
                    const response = await axios.patch(`http://localhost:3000/journey/updateWhatsAppCampaign/${selectedCampaign._id}`, campaignData);
                ...
                try {
                    const response = await axios.post(`http://localhost:3000/journey/uploadWhatsappCampaign`, campaignData);
    ...
    React.useEffect(()=>{
        if(selectedCampaign){
            setCampaignName(selectedCampaign.whatsAppCampaignName)
            CampaignTag(selectedCampaign.whatsAppCampaignTags)
            setProvider(selectedCampaign.whatsAppProvider)
            setSmsData({ subject: selectedCampaign.whatsAppNumber,content: selectedCampaign.whatsAppContent });
            setWhatsAppTemplate(selectedCampaign.whatsAppTemplate)
            setWhatsAppFile(selectedCampaign.whatsAppFile)
